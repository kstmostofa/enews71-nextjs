          ...
          <div className="col-span-12 md:col-span-4 border rounded-lg card-shadow bg-white">
            <Link href={`/${firstNews?.categories[0]?.slug}/${firstNews?.id}`}>
              <div className="">
                ...
                />
                <div className="p-4">
                  <h2 className="text-xl font-bold text-[#292219] cursor-pointer hover:underline hover:text-primary">
                    {firstNews?.title}
                  </h2>
                  <p className="text-slate-600 text-md mt-2 line-clamp-[9]">
                    {getStripHtml(firstNews?.content, 280)}...
                  </p>
                </div>
              </div>
            </Link>
                  ...
                  <Link key={index} href={`/${item.categories[0]?.slug}/${item.id}`}>
                    <div className="card-shadow border rounded-lg">
                      ...
                      <div className="py-2 px-4">
                        ...
                        <p className="text-slate-600 text-mx mt-2 line-clamp-[4]">
                ...
                return (
                  <Link key={index} href={`/${item.categories[0]?.slug}/${item.id}`}>
                    <div className="flex flex-row w-full border rounded-lg card-shadow">
                      ...
                      <div className="w-[70%] ">
                        <p className="text-md hover:underline text-[#292219] font-semibold p-2 hover:text-primary">
                          {item.title}
                        </p>
                      </div>
                    </div>
